...
---------------------------------------------------
* 隐藏层与输入同尺寸 (n×n)，每单元只接收 r=1 邻域 (9 像素)
* 几乎零噪声，输出阈值低，让输入驱动成为主导
"""
...
class LocalSpikingNet:
    def __init__(self, n:int=48, r:int=1, rho=0.9,
        ...
        self.M = self.N                    # 隐藏层 same size

        rng = np.random.default_rng(seed)
        self.W_in = np.zeros((self.M, self.N))
        # build sparse local connections, radius r
        ...
        # hidden -> output: identity (each pixel predicts itself 1‑step ahead)
    ...
    def update(self, X, s, out, target):
        err = target - out                         # {-1,0,1}
        # input->hidden Hebb (only potentiation)
        self.W_in += self.eta * np.outer(s, X)
        np.clip(self.W_in, 0, 1.5, out=self.W_in)
        # hidden->output error‑driven
...
def main():
    ...
    def update(t):
        ...
        fire, iou = net.diag()
        txt.set_text(f'Step {t} | fire {fire:.3f} | IoU₁₀₀ {iou:.2f}')
        X_prev = X_now
